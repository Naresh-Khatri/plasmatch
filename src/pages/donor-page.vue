<template>
  <div class="card">
    <div class="q-pa-md">
      <div v-if="regSuccessful" class="centerr">
        <q-stepper v-model="step" vertical color="primary">
          <h3 style="text-align: center; margin: 10px;">Thank you!</h3>
          <h3 style="text-align: center; margin: 10px;">For saving a Life </h3>
          <h5 style="text-align: center; margin: 10px;">
            We'll mail you at <span style="color: black; text-decoration: underline;">{{ email }}</span>
          </h5>
          <h5 style="text-align: center; margin: 10px;">
            or call you on
            <span style="color: black">{{ phoneNumber }}</span>
          </h5>
          <img
            src="../assets/donor-thank-you.gif"
            alt=""
            style="margin-left: 25%; width: 400px"
          />
          <div>
            <router-link to="/" style="text-decoration:none">
              <q-btn
                style="margin-left:50%"
                color="primary"
                label="Home"
                size="large"
              />
            </router-link>
          </div>
        </q-stepper>
      </div>
      <q-stepper v-model="step" vertical color="primary" animated v-else>
        <h4 class="q-ml-lg"><strong> Registration form </strong></h4>
        <q-form @submit="onSubmit" class="q-gutter-md">
          <q-step
            :name="1"
            title="Personal Information"
            icon="account_circle"
            :done="step > 1"
          >
            <dir class="row" style="padding:5px">
              <div class="col">
                <q-input
                  class="q-ma-sm"
                  outlined
                  v-model="firstName"
                  label="First Name"
                  lazy-rules
                  :rules="[
                    val =>
                      (val && val.length > 2) ||
                      'Please Enter Your Correct First Name'
                  ]"
                />
              </div>
              <br>
              <div class="col">
                <q-input
                  class="q-ma-sm"
                  outlined
                  v-model="lastName"
                  label="Last Name"
                  :rules="[
                    val =>
                      (val && val.length > 2) ||
                      'Please Enter your Correct Last Name'
                  ]"
                />
              </div>
            </dir>
            <!-- <q-input class='q-ma-sm'  outlined v-model="lastName" label="Age" type="number"/> -->
            <div class="row" :style="$q.screen.lt.md ? 'width:100px': '' ">
              <q-date
                v-model="dob"
                title="Date Of Birth"
              />
            </div>
            <div class="text-h6 q-mt-md">
              Gender
            </div>
            <div class="q-gutter-sm">
              <q-radio v-model="gender" val="F" label="Female" />
              <q-radio v-model="gender" val="M" label="Male" />
              <q-radio v-model="gender" val="O" label="Others" />
            </div>
            <div class="text-h6 q-mt-md">
              Blood Type
            </div>
            <div class="q-gutter-sm">
              <q-radio
                class="q-ml-md"
                v-model="bloodType"
                val="A+"
                label="A+"
              />
              <q-radio
                class="q-ml-md"
                v-model="bloodType"
                val="A-"
                label="A-"
              />
              <q-radio
                class="q-ml-md"
                v-model="bloodType"
                val="B+"
                label="B+"
              />
              <q-radio
                class="q-ml-md"
                v-model="bloodType"
                val="B-"
                label="B-"
              />
              <q-radio
                class="q-ml-md"
                v-model="bloodType"
                val="AB+"
                label="AB+"
              />
              <q-radio
                class="q-ml-md"
                v-model="bloodType"
                val="AB-"
                label="AB-"
              />
              <q-radio
                class="q-ml-md"
                v-model="bloodType"
                val="O+"
                label="O+"
              />
              <q-radio
                class="q-ml-md"
                v-model="bloodType"
                val="O-"
                label="O-"
              />
            </div>
            <q-stepper-navigation>
              <q-btn @click="step = 2" color="green-4" label="Continue" />
            </q-stepper-navigation>
          </q-step>

          <q-step
            :name="2"
            title="Health Information"
            icon="phone"
            :done="step > 1"
          >
            <div class="text-h6 q-mt-md">
              💊Do You Have Diabetes?
            </div>
            <div class="q-gutter-sm">
              <q-radio
                class="q-ml-md"
                v-model="diabetes"
                val="false"
                label="No"
              />
              <q-radio
                class="q-ml-md"
                v-model="diabetes"
                val="true"
                label="Yes"
              />
            </div>
            <div class="text-h6 q-mt-md">
              🍺Do You Have More Than 14 Drinks Per Week?
            </div>
            <div class="q-gutter-sm">
              <q-radio
                class="q-ml-md"
                v-model="moreThan14Drinks"
                val="false"
                label="No"
              />
              <q-radio
                class="q-ml-md"
                v-model="moreThan14Drinks"
                val="true"
                label="Yes"
              />
            </div>

            <div class="text-h6 q-mt-md">
              📝Do You Have A Discharge Report From The Hospital?
            </div>
            <div class="q-gutter-sm">
              <q-radio
                class="q-ml-md"
                v-model="dischargeReport"
                val="false"
                label="No"
              />
              <q-radio
                class="q-ml-md"
                v-model="dischargeReport"
                val="true"
                label="Yes"
              />
            </div>

            <div class="text-h6 q-mt-md">
              🦠Is Latest COVID NEGATIVE Test In The Last 2 Weeks?
            </div>
            <div class="q-gutter-sm">
              <q-radio
                class="q-ml-md"
                v-model="covidRecoveryInLast14Days"
                val="false"
                label="No"
              />
              <q-radio
                class="q-ml-md"
                v-model="covidRecoveryInLast14Days"
                val="true"
                label="Yes"
              />
            </div>

            <div class="text-h6 q-mt-md">
              🥩Do You Have Any Liver Disease?
            </div>
            <div class="q-gutter-sm">
              <q-radio class="q-ml-md" v-model="liver" val="false" label="No" />
              <q-radio class="q-ml-md" v-model="liver" val="true" label="Yes" />
            </div>

            <div class="text-h6 q-mt-md">
              Do You Have Any Kidney Disease?
            </div>
            <div class="q-gutter-sm">
              <q-radio
                class="q-ml-md"
                v-model="kidney"
                val="false"
                label="No"
              />
              <q-radio
                class="q-ml-md"
                v-model="kidney"
                val="true"
                label="Yes"
              />
            </div>

            <div class="text-h6 q-mt-md">
              🫁Do You Have Any Lung Disease?
            </div>
            <div class="q-gutter-sm">
              <q-radio class="q-ml-md" v-model="lung" val="false" label="No" />
              <q-radio class="q-ml-md" v-model="lung" val="true" label="Yes" />
            </div>

            <div class="text-h6 q-mt-md">
              🩸Do You Have High Blood Pressure?
            </div>
            <div class="q-gutter-sm">
              <q-radio
                class="q-ml-md"
                v-model="bloodPressure"
                val="false"
                label="No"
              />
              <q-radio
                class="q-ml-md"
                v-model="bloodPressure"
                val="true"
                label="Yes"
              />
            </div>
            <q-stepper-navigation>
              <q-btn flat @click="step = 1" color="primary" label="Back" />
              <q-btn @click="step = 3" color="green-4" label="Continue" />
            </q-stepper-navigation>
          </q-step>
          <q-step
            :name="3"
            title="Contact Information"
            icon="phone"
            :done="step > 2"
          >
            <q-input
              class="q-ma-sm"
              outlined
              v-model="email"
              label="Email"
              :rules="[
                val =>
                  (val &&
                    val.length > 10 &&
                    val.includes('@') &&
                    val.includes('.')) ||
                  'Invalid Email'
              ]"
            />
            <q-input
              class="q-ma-sm"
              outlined
              v-model="phoneNumber"
              label="Phone Number"
              :rules="[
                val =>
                  (val && val.length >= 10) ||
                  'Phone Number Contains 10 digits!'
              ]"
            />
            <q-stepper-navigation>
              <q-btn flat @click="step = 2" color="primary" label="Back" />
              <q-btn @click="step = 4" color="green-4" label="Continue" />
            </q-stepper-navigation>
          </q-step>
          <q-step
            :name="4"
            title="Address"
            caption=""
            icon="home"
            :done="step > 3"
          >
            <q-input
              class="q-ma-sm"
              outlined
              v-model="city"
              label="City"
              :rules="[
                val =>
                  (val && val.length > 5) || 'Please Enter a valid city name'
              ]"
            />
            <q-btn flat color="primary" label="Search" @click="findPlace" />
            {{ coordinates }}
            <GmapMap :center="coordinates" :zoom="9" id="map"> </GmapMap>

            <q-stepper-navigation>
              <q-btn flat @click="step = 3" color="primary" label="Back" />
              <q-btn @click="step = 5" color="green-4" label="Continue" />
            </q-stepper-navigation>
          </q-step>

          <q-step :name="5" title="Term & Conditions" icon="thumb_up_alt">
            <h3 style="margin-top:20px;">By submitting this form,</h3>
            <li>
              I confirm that the information I have provided here is complete
              and accurate to the best of my knowledge.
            </li>
            <li>
              I confirm that I fulfil the eligibility criteria for patients set
              out at https://Plasmatch.com.
            </li>
            <li>
              I confirm that I wish to share the information provided here with
              Plasmatch for the exclusive purpose(s) of matching with donors
              from the database of donors registered with Plasmatch and sharing
              this information directly and solely with the matched donors.
            </li>
            <li>
              I agree to hold harmless Plasmatch for its use of the information
              for the exclusive purpose(s) set out above.
            </li>
            <div class="p-t-15">
              <!-- <button class="btn btn--radius-2 btn--blue" type="submit">Confirm</button> -->
            </div>
            <q-stepper-navigation>
              <q-btn
                flat
                @click="step = 4"
                color="primary"
                label="Back"
                class="q-ml-sm"
              />
              <q-btn color="primary" label="Finish" type="submit" />
            </q-stepper-navigation>
          </q-step>
        </q-form>
      </q-stepper>
    </div>
  </div>
</template>

<script>
import axios from "axios";
export default {
  data() {
    return {
      info: {},
      firstName: "",
      lastName: "",
      age: Number,
      gender: "",
      email: "",
      city: "",
      phoneNumber: "",
      bloodType: "",
      dob: "",
      placeName: "",
      hadCovid: Boolean,
      covidRecoveryInLast14Days: Boolean,
      haveAadhar: Boolean,
      diabetes: Boolean,
      liver: Boolean,
      kidney: Boolean,
      lung: Boolean,
      bloodPressure: Boolean,
      moreThan14Drinks: Boolean,
      dischargeReport: Boolean,

      regSuccessful: false,
      step: 1,
      blockNumber: 0,
      // canSubmit: false

      coordinates: { lat: 0, lng: 0 }
    };
  },
  created() {
    this.askLocation();
  },
  methods: {
    onComplete: function() {
      alert("Yay. Done!");
    },
    askLocation() {
      this.$getLocation({})
        .then(coordinates => {
          this.coordinates = { lat: coordinates.lat, lng: coordinates.lng };
        })
        .catch(error => alert(error));
    },
    findPlace() {
      const request = {
        query: "Museum of Contemporary Art Australia",
        fields: ["name", "geometry"]
      };
      service = new google.maps.places.PlacesService(map);
      service.findPlaceFromQuery(request, (results, status) => {
        if (status === google.maps.places.PlacesServiceStatus.OK) {
          for (let i = 0; i < results.length; i++) {
            createMarker(results[i]);
          }

          map.setCenter(results[0].geometry.location);
        }
      });
    },
    onSubmit() {
      // this.regSuccessful = true;
      var from = this.dob.split("/");
      var old = from[2] * 86400 + (from[1] - 1) * 2592000 + from[0] * 31557600;
      var today = new Date();
      var curr =
        today.getDate() * 86400 +
        today.getMonth() * 2592000 +
        today.getFullYear() * 31557600;
      var diff = curr - old;
      this.age = Math.floor(diff / 31622400);

      console.log(this.age);
      var info = {
        email: this.email,
        first_name: this.firstName,
        last_name: this.lastName,
        dob: this.dob,
        age: this.age,
        gender: this.gender,
        phone_no: this.phoneNumber,
        city: this.city,
        lat: this.coordinates.lat,
        lng: this.coordinates.lng,
        blood_type: this.bloodType,
        diabetes: this.diabetes,
        lung: this.lung,
        kidney: this.kidney,
        high_bp: this.bloodPressure,
        liver: this.liver,
        have_aadhar: this.haveAadhar,
        drinks_14: this.moreThan14Drinks,
        discharge_report: this.dischargeReport,
        neg_2weeks: this.covidRecoveryInLast14Days
      };

      console.log(info);
      axios
        .post("https://apiv1.plasmatch.in/DonorProfile/", info)
        .then(response => {
          if (response.status === 201) {
            this.$q.notify({
              type: "positive",
              message: "Registration Successful!",
              position: "top",
              duration: 50
            });
            this.regSuccessful = true;
          }
        })
        .catch(error => {
          if (error.response) {
            console.log("bad requset");
            this.$q.notify({
              type: "negative",
              message: "Check your details or Email already exists!",
              position: "top",
              duration: 50
            });
          }
        });
    }
  },

  createMarker(place) {
    const marker = new google.maps.Marker({
      map,
      position: place.geometry.location
    });
    google.maps.event.addListener(marker, "click", () => {
      infowindow.setContent(place.name);
      infowindow.open(map);
    });
  }
};
</script>
<style>
#map {
  min-height: 500px;
  max-height: 500px;
  min-width: 300px;
  max-width: 700px;
}
.card {
  max-width: 800px;
  margin-left: auto;
  margin-right: auto;
  margin-top: 100px;
  border-radius: 5px;
}
.centerr {
  margin-left: auto;
  margin-right: auto;
}
</style>
